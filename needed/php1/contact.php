...
if (isset($_POST['send_contact'])) {
	...
	if (empty($firstname) || empty($secondname) || empty($email) || empty($message)) {
		$wrong = "all inputs can not be empty!";
	} else {
		$sql = "insert into contact (`firstname`,`lastname`,`email`,`message`) values ('$firstname','$secondname','$email','$message')";
		mysqli_query($link,$sql);
		$wrong = "<script>alert('your message have be sent!')</script>";
	}
}
					...
					<div class="col-md-6 contact-top-right">
						<div class="contact-textarea">
							<form method="post">
								...
								<input type="text" value="" name="secondname" placeholder="Second Name"/>
								<input type="text" value="" name="email" placeholder="Email"/>
								<textarea name="message" placeholder="Message"></textarea>
								<input type="submit" value="SUBMIT" name="send_contact"> 
